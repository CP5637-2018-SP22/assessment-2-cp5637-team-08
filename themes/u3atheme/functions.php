...
function u3atheme_setup() {
	...
	add_image_size( 'u3atheme-full-bleed', 2000, 1200, true );
	add_image_size( 'u3atheme-index-img', 1000, 550, true );

	// This theme uses wp_nav_menu() in one location.
	register_nav_menus( array(
		'primary' => esc_html__( 'Header', 'u3atheme' ),
		'social' => esc_html__( 'Social Media Menu', 'u3atheme' ),
	) );
	...
	/* Editor styles */
	add_editor_style( array( 'inc/editor-styles.css', u3atheme_fonts_url() ) );
}
endif;
...
function u3atheme_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'u3atheme' );
	$pt_serif = _x( 'on', 'PT Serif font: on or off', 'u3atheme' );
 ...
 */
function u3atheme_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
...
add_filter( 'wp_calculate_image_sizes', 'u3atheme_content_image_sizes_attr', 10, 2 );
 ...
 */
function u3atheme_header_image_tag( $html, $header, $attr ) {
	if ( isset( $attr['sizes'] ) ) {
		$html = str_replace( $attr['sizes'], '100vw', $html );
	}
	return $html;
}
add_filter( 'get_header_image_tag', 'u3atheme_header_image_tag', 10, 3 );
 ...
 */
function u3atheme_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'u3atheme_post_thumbnail_sizes_attr', 10, 3 );
...
function u3atheme_widgets_init() {
	register_sidebar( array(
		...
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add sidebar widgets here.', 'u3atheme' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Page Sidebar', 'u3atheme' ),
		'id'            => 'sidebar-2',
		'description'   => esc_html__( 'Add page sidebar widgets here.', 'u3atheme' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer Widgets', 'u3atheme' ),
		'id'            => 'footer-1',
		'description'   => esc_html__( 'Add footer widgets here.', 'u3atheme' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
...
function u3atheme_scripts() {
	...
	wp_enqueue_script( 'u3atheme-navigation', get_template_directory_uri() . '/js/navigation.js', array('jquery'), '20151215', true );
	...
	wp_enqueue_script( 'u3atheme-functions', get_template_directory_uri() . '/js/functions.js', array('jquery'), '20161201', true );
